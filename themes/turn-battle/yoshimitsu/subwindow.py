...
class DamageMessage(SubWindowBody):
    def draw(self, battlemanager: BattleManager):
        ...
        print(window_message["reduce_hp"].format(enemy.name, enemy.hp + enemy_damage, enemy.hp))
        
        print(window_message["new_line"])
        
        print(window_message["damage"].format(enemy.name, player.name, player_damage))
        print(window_message["reduce_hp"].format(player.name, player.hp + player_damage, player.hp))
...
class EnemyStatus(SubWindowBody):
    def draw(self, battlemanager: BattleManager):
        enemy = battlemanager.get_enemy()
        print(window_message["status"].format(enemy.name, enemy.lv, enemy.hp, enemy.at, enemy.de))

class PlayerStatus(SubWindowBody):
    def draw(self, battlemanager: BattleManager):
        player = battlemanager.get_player()
        print(window_message["status"].format(player.name, player.lv, player.hp, player.at, player.de))
